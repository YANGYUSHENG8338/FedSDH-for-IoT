...
tuple< MatrixXd, MatrixXd> AggregatedWWeight_server(MatrixXd RENEW_RASP_labelList, MatrixXd RENEW_NANO_labelList, MatrixXd RENEW_RASP_WWeight, MatrixXd RENEW_NANO_WWeight, int LabelCount, int paramL, double raspDataRatio, double nanoDataRatio)
{
    ...
    MatrixXd AGGREGATED_RASP_WWeight = AGGREGATED_WWeight;   // 創建 分發給 RASP 的 WWeight 矩陣
    MatrixXd AGGREGATED_NANO_WWeight = AGGREGATED_WWeight;   // 創建 分發給 NANO 的 WWeight 矩陣
...
//==========
    ...
    // // RASP 端找出全部 Label 種類減掉自己 Label 的種類的運算 結果放於RASP_NonlabelList
    ...
    // // NANO 端找出全部 Label 種類減掉自己 Label 的種類的運算 結果放於NANO_NonlabelList
