...
logPowerPath = str(param)             #logPowerPath '/home/raspdslab/evaPoolPath/logOneAggrePower.txt'
...
splitPathtxt = logPowerPath.split("/")       # 使用 "/" 进行分割

splitPathtxt[-1] = "Sum" + splitPathtxt[-1]  # 在最后一个子字符串前面加上 "Sum"

SumlogPowerPath = "/".join(splitPathtxt)     # 重新连接字符串    --- #SumlogPowerPath '/home/raspdslab/evaPoolPath/SumlogOneAggrePower.txt'
...
# 每 OneAggre 獲得一個平均值 ， 並將所有獲得之平均值，除以聚合的次數，獲得實驗結果
# print(result)
...
# CSV檔案路徑和名稱
# SumlogPowerPathcsv = "/home/raspdslab/evaPoolPath/logOneAggrePower.csv"


splitPathcsv = SumlogPowerPath.split("/")  # 使用 "/" 进行分割

splitPathcsv[-1] = splitPathcsv[-1].replace(".txt", ".csv")  # 将最后一个子字符串的 "txt" 替换为 "csv"

SumlogPowerPathcsv = "/".join(splitPathcsv)  # 重新连接字符串
